...
- `src/orchestrator/agent_cli.py` — `canopy agent …` CLI (`register`/`sync`/`work`/`skills`/`tasks-sync`/`commands`/`apply`/`set`), a thin shell over `AgentClient` for shell-driven agents. Registered on `main` in `cli.py`.
