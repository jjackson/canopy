...
- `canopy issue {create|context} ...` — architect-routed GitHub issues with `canopy.origin/v1` provenance. **`create`** (reads a JSON record on stdin) files a CLEAN, portable GitHub issue (mandate + done-criteria + a pointer — NO local paths/yaml) and stores the rich *understanding* (provenance, intent, evidence, POINTERS to the drilled sessions) as a record locally (best-effort sync to canopy-web; the `/api/issues` endpoint is a staged follow-up). **`context <owner/repo#n>`** hydrates that understanding + emits the `canopy harvest strip` commands to recover the local transcripts (flagging which exist on this machine — sessions stay local, never in the web record). Backed by `src/orchestrator/issue_origin.py`.
